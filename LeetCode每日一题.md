...
~~~c
// 查找函数：找到给定节点的根节点，并进行路径压缩
int Find(int* parent, int index) 
{
    // 如果当前节点不是其自身的父节点，则递归查找它的父节点，并将当前节点直接指向最终找到的根节点
...
// 合并函数：将两个节点所在的集合合并为一个
void Union(int* parent, int index1, int index2) 
{
    // 将index1所在集合的根节点的父节点设置为index2所在集合的根节点
    // 这样就完成了两个集合的合并
    parent[Find(parent, index1)] = Find(parent, index2);
}

// 主要逻辑函数：找出可以删除的一条边，使得剩下的部分构成一棵树
int* findRedundantConnection(int** edges, int edgesSize, int* edgesColSize, int* returnSize) 
{
    // 根据边的数量确定节点数量n（因为是无向图，所以节点数等于边数）
    int n = edgesSize;
    // 初始化父节点数组，大小为n+1，因为节点编号可能从1开始
    int parent[n + 1];
    // 初始化时，每个节点都是它自己的父节点
    for (int i = 1; i <= n; ++i) 
    {
    ...
    for (int i = 0; i < edgesSize; ++i) 、
    {
        ...
        {
            // 如果已经在同一个集合中，说明这条边形成了环，因此可以被删除
            *returnSize = 2;  // 设置返回数组的大小为2
            return edges[i];   // 返回这条多余的边
        }
    }

    // 如果遍历完所有边都没有发现环，则理论上不会发生这种情况
    // 但是为了符合函数签名要求，这里设置返回数组的大小为0，并返回NULL
    *returnSize = 0;
    return NULL;
}
~~~
...
- **功能**：查找给定索引`index`节点所属集合的根节点，并进行路径压缩。

- 参数：

  - `int* parent`：指向一个数组的指针，该数组存储了每个节点的父节点信息。
  - `int index`：要查找其根节点的节点索引。
  
- 过程：

  - 如果当前节点不是它自己的父节点（即`parent[index] != index`），那么递归调用`Find`函数来查找其父节点的实际根节点。
  - 执行路径压缩：将当前节点直接指向最终找到的根节点，以优化后续查找操作。
  - 返回最终找到的根节点。
...
- 参数：
  ...
  - `int* parent`：指向存储父节点信息的数组的指针。
  - `int index1, int index2`：需要被合并到同一集合中的两个节点的索引。

- 过程：

  - 首先找到`index1`和`index2`各自所在集合的根节点。
  - 然后将`index1`所在集合的根节点的父节点设置为`index2`所在集合的根节点，从而完成两个集合的合并。

**`findRedundantConnection` 函数**

- **功能**：在给定的边列表中找到一条可以删除的多余连接，使得剩下的部分构成一棵树。
- 参数：
  - `int** edges`：二维数组，表示一系列无向边。
  - `int edgesSize`：边的数量。
  - `int* edgesColSize`：每条边包含的元素数量（对于无向边通常是2）。
  - `int* returnSize`：输出参数，用于返回结果数组的大小。
- 过程：
  - 初始化父节点数组`parent`，其中每个节点初始时都是它自己的父节点。
  - 遍历每一条边：
    - 检查两条边连接的节点是否属于不同的集合（通过比较它们的根节点）。
    - 如果是，则合并这两个集合。
    - 如果发现两个节点已经属于同一个集合，说明添加这条边会形成环，因此这条边就是多余连接，立即返回这条边。
  - 如果遍历完所有边都没有发现环，则返回`NULL`（根据题目设定这种情况不会发生）。
